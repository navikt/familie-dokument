...
class GcpStorageConfiguration {
    ...
    ): GcpStorage = GcpStorage(bucketName, maxFileSizeMB * MAX_FILE_SIZE_FACTOR, storage)
    ...
    fun attachmentStorage(storage: GcpStorage): GcpStorageWrapper = GcpStorageWrapper(storage, MediaType.APPLICATION_PDF_VALUE)

    @Bean(STONAD_GCP_STORAGE)
    fun stonadStorage(storage: GcpStorage): GcpStorageWrapper = GcpStorageWrapper(storage, MediaType.APPLICATION_JSON_VALUE)
